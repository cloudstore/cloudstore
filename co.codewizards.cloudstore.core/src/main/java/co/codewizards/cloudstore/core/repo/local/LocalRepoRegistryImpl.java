...
public class LocalRepoRegistryImpl implements LocalRepoRegistry
{
	...
	private void setProperty(final String key, final Date value) {
		setProperty(key, new DateTime(requireNonNull(value, "value")).toString());
	}

	private String getProperty(final String key) {
		return repoRegistryProperties.getProperty(requireNonNull(key, "key"));
	}

	private void setProperty(final String key, final String value) {
		final Object oldValue = repoRegistryProperties.setProperty(requireNonNull(key, "key"), requireNonNull(value, "value"));
		if (!equal(oldValue, (Object) value))
			repoRegistryPropertiesDirty = true;
	}

	private void removeProperty(final String key) {
		if (repoRegistryProperties.remove(requireNonNull(key, "key")) != null)
			repoRegistryPropertiesDirty = true;
	}
	...
	private String getPropertyKeyForAlias(final String repositoryAlias) {
		return PROP_KEY_PREFIX_REPOSITORY_ALIAS + requireNonNull(repositoryAlias, "repositoryAlias");
	}

	private String getPropertyKeyForID(final UUID repositoryId) {
		return PROP_KEY_PREFIX_REPOSITORY_ID + requireNonNull(repositoryId, "repositoryId").toString();
	}
