...
public class FileWatcher {
	...
	private static final int TIMEOUT_SYNC = 10;
		...
		private void watchForCreationsDeletions() throws IOException {
			for (;;) {
				...
				for (final WatchEvent<?> event : key.pollEvents()) {
					...
					// the "too much" check:
					if (chunksCreated > toBeCreated || chunksDeleted > toBeDeleted) {
						throw new IllegalStateException("watchForCreationsDeletions: strange!" + " chunksCreated="
								+ chunksCreated + ", chunksDeleted=" + chunksDeleted);
					}
				}
				// reset key and remove from set if directory no longer
				// accessible
		...
		public Void call() throws Exception {
			...
			for (;;) {
				...
				for (final WatchEvent<?> event : key.pollEvents()) {
					...
					if (kind == StandardWatchEventKinds.ENTRY_CREATE) {
						System.err.println("watchForFileCreationAndSize: created=" + name);
						continue;
					}
					if (kind == StandardWatchEventKinds.ENTRY_MODIFY) {
						final long length = child.toFile().length();
						System.err.println("watchForFileCreationAndSize: modified=" + name + ", length=" + length);
						if (length == cumLength) {
